...
public class GeoJsonSerde implements SerDe {
				...
				if (token == JsonToken.START_OBJECT) {
					if (parser.getCurrentName() == "geometry") {
						if (geometryColumn > -1) {
                            ...
                            String geoJson = mapper.writeValueAsString(node);
                            System.err.println("TK GEOJSON:" + geoJson);
                            MapGeometry mapGeom = GeometryEngine.geometryFromGeoJson(geoJson,
                                                        GeoJsonImportFlags.geoJsonImportDefaults,
                                                        Geometry.Type.Unknown);
                            System.err.println("TK mapGeom:" + mapGeom.toString());
					...
					} else if (parser.getCurrentName() == "properties") {
